...
Original Code can be found at: https://github.com/EvansSchoolPolicyAnalysisAndResearch/337_Seasonal-Hunger-Malawi
Published Paper available at: https://evans.uw.edu/sites/default/files/Relating%20Seasonal%20Hunger%20and%20Prevention%20and%20Coping%20Strategies%20A%20Panel%20Analysis%20of%20Malawian%20Farm%20Households.pdf
More info on the University of Washington EPAR website: https://evans.uw.edu/policy-impact/epar/research/seasonal-hunger-and-coping-and-prevention-strategies-malawi
*/
...
*File Locations
global wd "yourfilepath"
global malawi_data	"$wd\Raw data"
...
use "$malawi_data\COM_CD_10.dta", clear
gen dist_dmarket = com_cd16a if com_cd16b==2				
	// kilometers is 2
replace dist_dmarket = com_cd16a*1.6 if com_cd16b==3		
	// miles is 3
replace dist_dmarket = com_cd16a/1000 if com_cd16b==1		
	// meters is 1
replace dist_dmarket = 0 if com_cd15==1						
	// equals zero if there is a market in the community
keep ea_id dist_*
tempfile mw1_comm
save `mw1_comm', replace


use "$malawi_data\HH_MOD_A_FILT_13.dta", clear
...
*Rainfall
use "$malawi_data\HouseholdGeovariables_IHS3_Rerelease_10.dta", clear
ren case_id hhid
gen rain_ratio2009 = (h2009_tot-anntot_avg)/anntot_avg									
	// percent deviation
...
use "$malawi_data\HH_MOD_A_FILT_10.dta", clear
...
gen month = month1 if qx_type=="Panel A"												
	// Correct interview date is different for different waves
gen year = year1 if qx_type=="Panel A"
replace month = month2 if qx_type=="Panel B"
replace year = year2 if qx_type=="Panel B"

gen ym_int = ym(year,month)																
	// Month in Stata format
format ym_int %tm
gen wave=1
...
use "$malawi_data\HH_MOD_B_10.dta", clear
merge 1:1 case_id PID using "$malawi_data\HH_MOD_C_10.dta", nogen assert(3)				
	// Merging for education
ren case_id hhid
gen personid = hh_b01
...
gen educ_head = educ if relationship==1 			
	// Assuming Training College follows Form 6 and is equivalent to University 1, 2, 3, 4
...
*Labor variables
use "$malawi_data\HH_MOD_E_10.dta", clear
ren case_id hhid
...
use "$malawi_data\HH_MOD_N2_10.dta", clear
merge m:1 case_id using "$malawi_data\HH_MOD_N1_10.dta", nogen
ren case_id hhid
gen ent_count = 1
replace ent_count = 0 if hh_n0b==2														
	// = 0 if household did NOT operate one
gen ent_sales = hh_n32
replace ent_sales = 0 if hh_n0b==2														
	// = 0 if household did NOT operate one
...
*Sales of Crops
use "$malawi_data\AG_MOD_I_10.dta", clear
ren case_id hhid
gen sales_dum = ag_i01==1
gen sales_value = ag_i03																
	// "9999999" is assumed to be top-coded, not a missing value
...
use "$malawi_data\AG_MOD_Q_10.dta", clear
ren case_id hhid
gen sales_dum_perm = ag_q01==1
gen sales_value_perm = ag_q03															
	// "9999999" is assumed to be top-coded, not a missing value
...
use "$malawi_data\AG_MOD_B_10.dta", clear
ren case_id hhid
gen ag2008 = ag_b0a==1																	
	// Those that actually planted crops in 2008/2009 = 1
...
use "$malawi_data\AG_MOD_C_10.dta", clear
ren case_id hhid
ren ag_c00 plotid
...
use "$malawi_data\AG_MOD_D_10.dta", clear
ren case_id hhid
ren ag_d00 plotid
ren ag_d01 personid
gen orgfert = ag_d36 ==1
gen inorgfert = ag_d38 ==1
merge m:1 personid hhid using `mw1_inddemog', keepusing(relationship male age) nogen keep(1 3)		
	// Not keeping unmatched from using
*Fixing a duplicate
...
use "$malawi_data\AG_MOD_G_10.dta", clear
ren case_id hhid
...
drop maize
ren ag_g0b plotid
merge m:1 hhid plotid using `mw1_plot', keep(3) nogen		
	// 64 not matched from using (keeping only matched)
*Identifying crops
gen maize = inlist(ag_g0d,1,2,3,4)
...
bys hhid plotid: egen total_prop = sum(proportion)	
	// Getting TOTAL proportion, which is not always one
replace proportion = proportion/total_prop if total_prop>1			
	// Now they MUST sum to one IF they are over one

*Here we tag just one of each crop (e.g. one of the 10 rice varietals)
foreach i of varlist maize-other{
	gen `i'_temp = 1 if `i'==1
	egen `i'_tag = tag(`i'_temp hhid)		
		// This variable tags just one of each varietal
}
...
*Livestock
use "$malawi_data\AG_MOD_R1_10.dta", clear
ren case_id hhid
...
*Fruit/Perm Crops
use "$malawi_data\AG_MOD_P_10.dta", clear
ren case_id hhid
tab ag_p0a
gen any_perm = ag_p0d!=.
...
use "$malawi_data\Round 1 (2010) Consumption Aggregate.dta", clear // FILE NOT FOUND
ren case_id hhid
...
use "$malawi_data\HH_MOD_X_10.dta", clear
ren case_id hhid
...
use "$malawi_data\FS_MOD_B_FILT_10.dta", clear
ren case_id hhid
...
************************
*Community (for distance variables)
use "$malawi_data\COM_MOD_D_13.dta", clear
...
*Rainfall
use "$malawi_data\HouseholdGeovariables_IHPS_13.dta", clear
...
*Household demographics
use "$malawi_data\HH_MOD_B_13.dta", clear
*For education
merge 1:1 y2_hhid PID using "$malawi_data\HH_MOD_C_13.dta", nogen 					// 1 not matched from master
ren PID pid
...
*Labor variables
use "$malawi_data\HH_MOD_E_13.dta", clear
gen any_wage = hh_e18==1 | hh_e55==1
...
use "$malawi_data\HH_MOD_N2_13.dta", clear
merge m:1 y2_hhid using "$malawi_data\HH_MOD_N1_13.dta", nogen
gen ent_count = 1
...
use "$malawi_data\AG_MOD_BA_13.dta", clear
...
use "$malawi_data\AG_MOD_I_13.dta", clear
gen sales_dum = ag_i01==1
...
*Sales of Crops - PERMANENT
use "$malawi_data\AG_MOD_Q_13.dta", clear
gen sales_dum_perm = ag_q01==1
...
*THIS PART USES PLOT MODULE INSTEAD OF CROP MODULE
use "$malawi_data\AG_MOD_C_13.dta", clear
ren ag_c00 plotid
gen acres_gps = ag_c04c
gen acres_report = ag_c04a if ag_c04b==1
gen acres = acres_gps
...
*Now the other plot info
use "$malawi_data\AG_MOD_D_13.dta", clear
ren ag_d00 plotid
ren ag_d01 personid
gen orgfert = ag_d36 ==1
gen inorgfert = ag_d38 ==1
...
*And now the plot-crop level
use "$malawi_data\AG_MOD_G_13.dta", clear
gen month_start = ag_g12a
gen month_end = ag_g12b
...
*Livestock
use "$malawi_data\AG_MOD_R1_13.dta", clear
...
*Fruit/Perm Crops
use "$malawi_data\AG_MOD_P_13.dta", clear
...
*Consumption Aggregates
use "$malawi_data\Round 2 (2013) Consumption Aggregate.dta", clear
gen total_exp = rexpagg
gen total_exp_cap = pcrexpagg
...
use "$malawi_data\HH_MOD_X_13.dta", clear
...
*Remittances
use "$malawi_data\HH_MOD_P_13.dta", clear
egen total_received = rowtotal(hh_p03a hh_p03b hh_p03c)
collapse (sum) total_received, by(y2_hhid)
tempfile remit
save `remit', replace
use "$malawi_data\HH_MOD_O_13.dta", clear
gen remittances = hh_o14
...
*Adding FISHERY PRODUCTIOn
use "$malawi_data\FS_META_13.dta", clear
gen fish = 1
keep y2_hhid fish
tempfile mw2_fish
...
************************
************************
use "$malawi_data\HH_MOD_H_10.dta", clear
ren case_id hhid
...
************************
************************
use "$malawi_data\HH_MOD_H_13.dta", clear
gen hunger_any = hh_h04==1
...
*Some recoding
recode acres* crop_count* (.=0)			
	// Assuming missing values for these variables means zero 
	// (e.g. no ag module therefore missing crop count)
...
*******************************************************
save Malawi_Hunger_EPAR.dta, replace
*******************************************************
...
*******************************************************
save Malawi_Hunger_EPAR_allvars.dta, replace
*******************************************************

* Keep only variables useful for student exercise
keep hhid ea_id stratum region district weight split ta rural month ym_int wave acres orgfert inorgfert acres_planted crop_count hhsize children adult_male adult_female age_head male_head educ_head bovine_num goat_num sheep_num pig_num poultry_num ox_num equine_num livestock_any sales_count sales_value sales_dum stored_annual sales_count_perm sales_value_perm sales_dum_perm stored_perm ent_count ent_sales nfe_any wage_count any_wage crop_count_previous other_num ym_harv_prev month_start_maize_prev month_start_prev month_end_prev ym_harv_prev_end month_end_maize_prev herf_previous ym_harv_first ym_maize_first_date ym_maize_first month_start month_start_maize ym_harv_last ym_maize_last month_end month_end_maize any_perm_food fruit crop_count_perm ag_hh remittances dist_road agrozone fish rain2010_tot wave_count first_harv last_harv first_harv_month last_harv_month prev_first_harv prev_last_harv simpson_previous first_harv_wave1 last_harv_wave1 pre_harv_hunger pre_harv_hunger_ALL hunger chronic_month_hunger chronic_month_hunger_ALL log_acres_p hectares hectares_p FE dist hhidfe panel_weight mammal poultry rain stored_crop ea_wave hhfe integer_weight any_hunger
drop month panel_weight integer_weight first_harv_wave1 last_harv_wave1 sales_dum stored_annual sales_count_perm sales_value_perm stored_perm ent_sales crop_count_previous ym_harv_prev ym_harv_prev_end herf_previous ym_harv_first ym_maize_first_date ym_harv_last fruit fish rain2010_tot first_harv last_harv first_harv_month last_harv_month prev_first_harv prev_last_harv pre_harv_hunger_ALL chronic_month_hunger chronic_month_hunger_ALL	
	
* Additional variable labels
lab var ym_int "Date (Year-Month)"
lab var rural "Rural/Urban"
	cap lab def rural 0 "Urban" 1 "Rural"
	lab val rural rural
lab var wave "Survey Wave (1=2010, 2=2013)"
lab var ag_hh "Farming Household"
	cap lab def farms 0 "Household is not engaged in agriculture" 1 "Household engaged in agriculture"
	lab val ag_hh farms
lab var ox_num "Oxen (count)"
lab var wave_count "Number of Waves observed"
lab var hectares "Total hectares owned"
lab var hectares_p "Hectares planted"
lab var weight "Survey weight"
lab var ea_wave "EA Fixed Effect"
lab var hhfe "Household Fixed Effect"

* Label all dummies
cap lab def yesno 0 "No" 1 "Yes"
lab var split "Household is a split-off in wave 2"
	lab val split yesno
lab var livestock_any "Any livestock" 
	lab val livestock_any yesno
lab val inorgfert yesno
lab val orgfert yesno
lab var nfe_any "Any non-farm enterprises"
	lab val nfe_any yesno
lab var any_hunger
	
cap drop _*
save Malawi_Hunger_EPAR, replace
